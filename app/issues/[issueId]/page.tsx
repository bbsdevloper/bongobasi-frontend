...
const Issue = ({ params }: any) => {
    ...
    const daysPassed: any =
        issue?.issuedate && intervalToDuration({ start: parseInt(issue?.issuedate), end: Date.now() })
    const { user } = useUser()
        ...
        <div className="min-h-screen bg-blueBackground">
            ...
            <div className="grid grid-cols-4  min-h-[90vh] bg-white rounded-xl shadow-custom mx-8 my-8 divide-x-2 ">
                <div
                    ...
                    } ${issue?.issuelevel === 'low' && 'border-t-blue-500'} border-t-8`}
                >
                    <div className=" px-6 space-y-4 py-4 ">
                        ...
                        <section className="flex justify-start items-center">
                            ...
                            <span
                                ...
                                } ${issue?.issueprogress === 'finished' && 'text-green-600'} font-semibold`}
                            >
                                {issue?.issueprogress}
                            </span>
                        </section>
                    ...
                    <div>
                        ...
                        <button
                            onClick={async () => {
                                ...
                                console.log(issue)
                                setIssue((prev: any) => {
                                    prev?.issuecomments.push(commentData as ICommentData)
                                    return prev
                                    // return {
                                    //     ...prev,
                    ...
                    <div className="px-4 py-4 space-y-4 ">
                        ...
                        <p className="text-xl font-semibold ">{issue?.issuecomments?.length} Messages</p>
                        {issue?.issuecomments?.filter((data) => data.commenttype == 'citizen').length !== 0 ? (
                            issue?.issuecomments
                                        ...
                                        <div key={id}>
                                            <div className="flex justify-start items-center">
                                                ...
                                                <div className="flex flex-col justify-start px-2 leading-snug font-semibold ">
                                                    <span className="text-green-800 text-sm ">{data.username}</span>
                                                    <p>{data.body}</p>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div>
                    <h1 className=" text-center font-semibold text-2xl py-3 text-red-700">Updates</h1>
                    {/* All Issue Updates */}
                            ...
                            <div key={id}>
                                {data.commenttype == 'officer' && (
                                    <div className="flex flex-col justify-start px-2 gap-2 font-semibold ">
                                        <p>
                                            {data.body}{' '}
                                            <span className="text-green-800 text-xs  px-2"> -{data.username} </span>
                                        </p>
                                    </div>
                                )}
                            </div>
